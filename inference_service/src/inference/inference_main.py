...
flags.DEFINE_string("output_dir", "./prediction/", "Path to results.")
...
def merge_all_geojson_to_one(gj_list):
...
def main(argv):
    ...
    image_Paths = load_images(PATH_IMAGE)
    ...
    # Load saved model and build the detection function
    model_path = os.path.join(PATH_MODELS, f'{argv[0]}/saved_model/')
    ...
    #     print("detections",detections)   
        ...
        pred_tile = detection_to_dictionary(task_id, bboxes, selected_classes_np, selected_scores_np)
        features.append(pred_tile)
    ...
    geojson_all = merge_all_geojson_to_one(features)

    # output_path = f"{PATH_OUTPUT}merged_prediction.geojson"
